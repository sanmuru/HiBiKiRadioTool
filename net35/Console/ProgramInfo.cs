...
namespace HiBikiRadioTool
{
    ...
    {
        private readonly program jObject;
        ...
        public string[] Hashtags => (
                                     ...
                                     where match.Success
                                     let hashtagWithoutHash = match.Value.TrimEnd()
                                     where hashtagWithoutHash.Length > 1
                                     select $"#{hashtagWithoutHash.Substring(1)}"
                                    ).ToArray();
        ...
        public ProgramInfo(program jObject) => this.jObject = jObject ?? throw new ArgumentNullException(nameof(jObject));
